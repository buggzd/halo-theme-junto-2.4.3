...
从 2.4.3 升级时，即使尚未保存新增设置，新版本也会先使用安全的默认文案，不会出现空白区块。2.4.15 在 `junto-blue-archive-template 2` 版式上加入 Blue Phantom 视觉系统，保留原 conic 渐变 Hero，优化 Anton 大标题的行高与字距，恢复全站交互元素的反色光标，并继续覆盖 Sakura 首页 Header hover 定位冲突。主题兼容 Halo 2.20.14。
