...
从 2.4.3 升级时，即使尚未保存新增设置，新版本也会先使用安全的默认文案，不会出现空白区块。2.4.10 按 `junto-blue-archive-template 2` 最终实验版恢复浮动导航、全屏搜索、时钟、圆形过场、首页空间排版与交互动效，并通过隔离首屏绘制上下文彻底修复首页导航的 Chrome 合成纹理错位，同时保留鼠标跟随泛光。主题兼容 Halo 2.20.14。
