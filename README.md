<h1 align="center">Junto Blue Archive</h1>

> 基于 Halo Theme Sakura 2.4.3 改造的深蓝编辑风个人创作档案主题。视觉设计来自仓库同级的 `junto-blue-archive-template 2`，保留 Sakura 对 Halo 2.x、PJAX、评论、搜索、图库、瞬间和友链插件的兼容能力。

## 当前进度

- [x] 独立主题标识与构建路径（`theme-junto`）
- [x] Junto 全局深蓝视觉系统、噪点材质和响应式排版
- [x] 新首页首屏、滚动信号条和动态文章索引
- [x] 首页首屏、滚动文字、入口、作品、项目与文章区文案可在“Junto 专属”中自定义
- [x] 新导航与页脚
- [x] 文章正文的编辑风排版
...
- Halo 2.20.14 推荐插件版本：搜索 `1.7.1`、图库 `1.4.0`、友链 `1.5.0`、瞬间 `1.9.0`
- 评论、搜索、图库、友链和瞬间均为可选插件；未安装图库插件时，首页会显示明确的空状态

## 安装与更新

1. 从 GitHub Releases 下载最新的 `halo-theme-junto-*.zip`。
2. 进入 Halo 后台的“外观 → 主题”，选择“安装主题”并上传 ZIP；已经安装旧版时选择升级即可。
3. 启用主题后，进入“外观 → 主题 → Junto Blue Archive → 设置 → Junto 专属”。
4. 首页首屏描边字、滚动文字、三个入口、作品、项目和文章区文案均可在这里修改；站点名称和副标题继续使用 Halo 的站点基本设置。

从 2.4.3 升级时，即使尚未保存新增设置，新版本也会先使用安全的默认文案，不会出现空白区块。2.4.12 按 `junto-blue-archive-template 2` 最终实验版恢复浮动导航、全屏搜索、时钟、圆形过场、首页空间排版与交互动效，覆盖 Sakura 仅在首页 hover 时切换 Header 定位方式的遗留规则，并将 Projects 页面中的实现说明改为简洁的作品集文案。主题兼容 Halo 2.20.14。
...
生产构建：`pnpm build`。生成文件位于 `templates/assets/dist`。

## 上游说明

本项目保留 Sakura 的 MIT 许可证及上游署名。以下为原项目文档。
